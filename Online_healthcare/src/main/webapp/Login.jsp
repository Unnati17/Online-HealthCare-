<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head><title>HealthCare System | Login</title>

<link rel="stylesheet" href="/mystyle.css" type="text/css" />
</head>
<body>
<form method="post" action="/Online_healthcare/LoginHandler" name="form1">
<table width="900" align=center>
	<tr>
	<td colspan="2"><%@ include file="/people_header.jsp" %></td>
    </tr>
	<tr >
	<td colspan="2">
	<table border ="0"  align="center" >
	<tr><td>E-mail</td>
	<td align="center" ><input type ="text" name="email" id="email" value="" /></td></tr>
	<tr><td>Password</td>
	<td align="center" ><input type ="password" name="pass" id="pass" value="" /></td></tr>
	<tr><td>Authentication id</td>
	<td align="center" ><input type ="text" name="auth_id" id="auth_id" value="" /></td></tr>
	<tr><td colspan="2" align="center" ><input type ="submit" name="submit" id="submit" value="Submit" />
	<input type="hidden" name="action_submit" id="action_submit" value="user_login_submit"/>
<!-- </td></tr>
	<tr><td colspan="2" align="center">
	<input type ="submit" name="submit" id="submit" value="Change Password" />
	<input type="hidden" name="action_chngpswd" id="action_chngpswd" value="people_change_pswd_submit"/>
	</td></tr> -->
	<%
	String msg  = (String)session.getAttribute("lErrorMsg");
	if (msg!=null && msg.length()>0 ){
	%>
    <tr>
	<td colspan="2" align="center"> 
	<%	out.println("<div class=boldred>"+msg+"</div>"); %>
	</td>
	</tr>
	<%	}	%>
	</table>
	</td></tr>
	<tr>
		<td colspan="2"><%@include file="/people_footer.jsp"%></td>
	</tr>
	</table>
</form></body></html> 